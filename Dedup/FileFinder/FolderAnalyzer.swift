    ...
    func analyzeFolderStats(url: URL, into fileSetBySize: FileSetBySize, completion: (() -> Void)? = nil)
    {
        ...
        DispatchQueue.global(qos: .userInitiated).async
        {
            ...
            )
            {
                ...
                {
                    // create MediaFile and let it get the rest of the info
                    guard let mediaFile = MediaFile(fileUrl: fileURL) else {
                        // Failed to create MediaFile - skip this file
                        continue
                    }
